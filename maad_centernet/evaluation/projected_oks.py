...
def compute_projected_oks(self, imgId, catId, mode):
    ...
    if mode == "kp_stem":
        ...
        project_inds = []
    ind = np.array(ind)
    ind = np.transpose(np.array([ind * 3, ind * 3 + 1, ind * 3 + 2])).reshape(
        -1
    )
    p = self.params
    ...
    for i, dt in enumerate(dts):
        ...
        for j, gt in enumerate(gts):
            ...
            for project_ind in project_inds:
                t_j = np.array(
                    [g[(project_ind) * 3], g[(project_ind) * 3 + 1]]
                )
                t_j_prev = np.array(
                    [g[(project_ind - 1) * 3], g[(project_ind - 1) * 3 + 1]]
                )
                t_j_next = np.array(
                    [g[(project_ind + 1) * 3], g[(project_ind + 1) * 3 + 1]]
                )
                if np.all((t_j_prev == t_j_next)):
                    ...
                    g_temp[project_ind * 3 : project_ind * 3 + 2] = (
                        get_closest_point_on_segment_list(p, a, b)
                    )
            d_temp = d[ind]
            g = g_temp[ind]
            xd = d_temp[0::3]
