...
from maad_centernet.config import Exp, get_exp
...
def make_parser():
    ...
    parser.add_argument(
        "-ckpt", "--weights", default=None, type=str, help="checkpoint file"
    )

    parser.add_argument(
        "-img", "--image", default=None, type=str, help="image file"
    )

    parser.add_argument(
        "--logger", default="wandb", type=str, help="tensorboard | wandb"
    )
    parser.add_argument(
        "opts",
...
def denorm(exp, predictions, device):
    output_size = (
        exp.input_size[0] / exp.down_ratio,
        exp.input_size[0] / exp.down_ratio,
    )
    if "obb" in predictions:
        ...
        predictions["obb"] *= (
            denorm.unsqueeze(1).unsqueeze(1).expand(predictions["obb"].shape)
        )
...
def main(exp: Exp, args):
    ...
    augmentation_list1.append(
        A.LongestMaxSize(max_size=exp.input_size[0], interpolation=1, p=1.0)
    )
    augmentation_list1.append(
        A.PadIfNeeded(
    ...
    # Make Prediction
    if exp.target_mode_conf["do_da"]:
        predictions, loss_stats, _, _ = model(img_normalized, None, None)
    else:
        predictions, loss_stats, _, _ = model(img_normalized, None)

    evaluator = exp.get_evaluator(args)
    (performance, loss_meter), bb_detections, kp_detections = exp.eval(
        model, evaluator
    )
    print(performance)
    ...
    if "kp" in predictions.keys():
        ...
        kps = kp_decode(
            predictions["hm"],
            predictions["kp"],
            exp.target_mode_conf["angle_mode"],
            kphm=kphm,
            reg=predictions["off"],
        )
