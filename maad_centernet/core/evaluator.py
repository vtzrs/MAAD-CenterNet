...
from maad_centernet.evaluation import dota_mAP
from maad_centernet.evaluation import compute_projected_oks
...
class Evaluator:
    def __init__(self, exp, args):
        ...
        self.experiment_path = os.path.join(
            self.exp.output_dir, args.experiment_name
        )
        if self.exp.debug:
    ...
    def generate_dota_annotations(self):
        ...
        for img_id in range(len(dataset)):
            ...
            for ann_id in range(bbs.shape[0]):
                ...
                line += "0"
                lines.append(line)
            img_str = (os.path.basename(target["img_id"])).replace(
                ".png", ".txt"
            )
    ...
    def dota_eval(self, detections):
        ...
        for img_id, detection in enumerate(detections):
            ...
            for obbox in detection:
                ...
                if score > self.exp.center_thresh:
                    ...
                    # Convert oriented bbox to straight bbox
                    obbox_corners = obbox_2_box_corners(
                        obbox[0], obbox[1], tlbr, obbox[4]
                    )
                    for corner in obbox_corners:
        ...
        ap50, info = dota_mAP(
            detpath, annopath, imagesetfile, self.exp.classes
        )
    ...
    def generate_coco_annotations(self):
        ...
        for img_id in range(len(dataset)):
            ...
            for ann_id in range(bbs.shape[0]):
                ...
                # Convert oriented bbox to straight bbox
                bbox_corner = obbox_2_box_corners(
                    bbox[0], bbox[1], tlbr, bbox[4]
                )
                ...
                if len(pl_i) > 0:
                    pl_i = np.concatenate(
                        (pl_i, np.ones((pl_i.shape[0], 1)) * 2), axis=1
                    )
        ...
        json.dump(
            coco_anns, open(f"{self.coco_output_path}/instances_val.json", "w")
        )


    def coco_pytool_eval(
        self, bb_detections, kp_detections, projected=False, mode="kp_all"
    ):
        coco_dets = []
                ...
                for kp_detection in kp_detections[img_id]:
                    ...
                    if score > self.exp.center_thresh:
                        ...
                        kp_detection = np.concatenate(
                            (
                                kp_detection,
                                np.ones((kp_detection.shape[0], 1)) * 1,
                            ),
                            axis=1,
                        )
        ...
        json.dump(
            coco_dets,
            open("{}/results.json".format(self.coco_output_path), "w"),
        )

        cocoGt = coco.COCO(self.coco_output_path + "/instances_val.json")
        coco_dets = cocoGt.loadRes(
            "{}/results.json".format(self.coco_output_path)
        )
        if len(bb_detections) > 0:
    ...
    def evaluate(self, model):
        ...
        for iter, batch in enumerate(self.val_loader):
            iter_bb_detections, iter_kp_detections = self.eval_iter(
                model, iter, batch
            )
    ...
    def denorm(self, predictions, targets):
        ...
        if "kp" in predictions:
            ...
            mult = (
                torch.cat([denorm_values_dist, *denorm_values_angle], dim=1)
                .reshape(-1)
                .to(self.device)
            )
            targets["kp"] *= mult
    ...
    def eval_iter(self, model, iter, batch):
        self.exp.target_mode_conf["do_da"] = False
        iter_start_time = time.time()

        targets = {
            k: batch[k] for k in batch.keys() if k != "img" and k != "meta"
        }
        ...
        predictions, loss_stats, _, _, _, _ = model(imgs, targets)
        ...
        if self.exp.debug:
            ...
            self.imwrite_debug_images(
                iter, imgs, targets, predictions, dets, kps, batch["meta"]
            )
        ...
        self.exp.target_mode_conf["do_da"] = True
        return dets, kps
    ...
    def imwrite_debug_images(
        self, iter, imgs, targets, predictions, dets, kps, meta
    ):
        for k in range(imgs.shape[0]):
            # Get kth datapoint and its prediction
            img = imgs[k, :, :, :]
            if self.exp.p_normalize:
                img = ImageDrawer.tensor_to_cv2_img(
                    img, ds_mean=self.exp.ds_mean, ds_std=self.exp.ds_std
                )
            else:
                img = ImageDrawer.tensor_to_cv2_img(img)
            target = {key: targets[key][k] for key in targets.keys()}
            prediction = {
                key: predictions[key][k] for key in predictions.keys()
            }
            if len(dets) > 0:
                det = dets[k, :, :]
            if len(kps) > 0:
            ...
            # gt_hm = self.image_drawer.draw_heat_map(img.copy(), target["hm"]) # remove heatmap
            pred_img = img.copy()
            if len(dets) > 0:
                pred_img = self.image_drawer.draw_predicted_bb(pred_img, det)
            if len(kps) > 0:
                pred_img = self.image_drawer.draw_pred_kps(pred_img, kp)
            # pred_hm = self.image_drawer.draw_heat_map(
            #     img.copy(), prediction["hm"]
            # ) # remove heatmap
            image_row_1 = gt_img
            image_row_2 = pred_img
            combined_image = cv2.vconcat([img, image_row_1, image_row_2])
            combined_image = cv2.cvtColor(combined_image, cv2.COLOR_RGB2BGR)
            cv2.imwrite(
                os.path.join(self.debug_path, f"sample_{iter}_{k}.jpg"),
                combined_image,
            )
            os.makedirs(
                os.path.join(self.debug_path, "predictions"), exist_ok=True
            )
            ...
            cv2.imwrite(
                os.path.join(
                    self.debug_path,
                    "paper_imgs",
                    f"{os.path.basename(meta['img_id'][k])}",
                ),
                pred_img,
            )
