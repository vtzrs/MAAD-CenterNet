...
from maad_centernet.utils import obbox_2_box_corners
...
class ImageDrawer:
    ...
    def draw_gt_kps(self, img, target):
        ...
        for index, offset, kp in zip(
            target["ind"], target["off"], target["kp"]
        ):
            ...
            if index == 0:
                break
            cx = (
                index % (img_w // self.down_ratio) + offset[0]
            ) * self.down_ratio
            cy = (
                math.floor(index // (img_w // self.down_ratio)) + offset[1]
            ) * self.down_ratio
        ...
        for score, stem in zip(scores, stems):
            if score[0] > self.center_thresh:
                for stem_point in stem:
                    cv2.circle(
                        img,
                        (int(stem_point[0]), int(stem_point[1])),
                        2,
                        self.colors["kp"],
                        7,
                    )
    ...
    def draw_ground_truth_bb(self, img, target):
        ...
        for i, (index, offset, bb) in enumerate(
            zip(target["ind"], target["off"], target["obb"])
        ):
            index = index.cpu().numpy()
            bb = bb.cpu().numpy()
            offset = offset.cpu().numpy()
            ...
            cv2.drawContours(
                img, [bb_corners.astype(int)], 0, self.colors["bbox"], 2
            )
        return img
    ...
    def _draw_predicted_bb(self, img, bb, cat, conf=1, show_txt=True):
        ...
        cv2.circle(
            img,
            (round(x1 + (x2 - x1) / 2), round(y1 + (y2 - y1) / 2)),
            5,
            self.colors["bbox"],
            2,
        )
    ...
    def _draw_predicted_bb_oriented(
        self, img, bb, angle, cat, conf, show_txt=True
    ):
        bb = bb.detach().cpu().numpy()
        bb = np.array(bb, dtype=np.int32)
        ...
        cv2.drawContours(
            img, [bb_corners.astype(int)], 0, self.colors["bbox"], 5
        )
        if show_txt:
            ...
            rot_mat = cv2.getRotationMatrix2D(
                [float(cx), float(cy)], -angle, 1.0
            )
            cat_img = cv2.warpAffine(
                cat_img, rot_mat, cat_img.shape[1::-1], flags=cv2.INTER_LINEAR
            )
    ...
    def draw_predicted_bb(self, img, det):
        ...
        for d in det:
            if d[5] > self.center_thresh:
                img = self._draw_predicted_bb_oriented(
                    img, d[:4], d[4], d[-1], d[5]
                )
        return img
