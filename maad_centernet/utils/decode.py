...
from maad_centernet.utils.model_utils import (
    _gather_feat,
...
def kp_decode(
    ...
    kps = kp_new
    if kphm is not None:
        kps = map_regression_to_heatmap(
            kphm, kps, batch, num_kps, width, device, K, kp_hm_offset
        )
...
def map_regression_to_heatmap(
    kphm, kps, batch, num_kps, width, device, K, kphm_offset
):
    kphm = _nms(kphm)
    if kphm.shape[1] == 5:
        kp_hm_extend = torch.zeros(
            (kphm.shape[0], 8, kphm.shape[2], kphm.shape[3])
        ).to(device)
    ...
    thresh = 0.1
    kps = (
        kps.view(batch, K, num_kps, 2).permute(0, 2, 1, 3).contiguous()
    )  # b x J x K x 2
    ...
    if kphm_offset is not None:
        hp_offset = _transpose_and_gather_feat(
            kphm_offset, hm_inds.view(batch, -1)
        )
    ...
    hm_kps = (
        ...
        .unsqueeze(2)
        .expand(batch, num_kps, K, K, 2)
    )
    dist = ((reg_kps - hm_kps) ** 2).sum(dim=4) ** 0.5
    min_dist, min_ind = dist.min(dim=3)  # b x J x K
    hm_score = hm_score.gather(2, min_ind).unsqueeze(-1)  # b x J x K x 1
    min_dist = min_dist.unsqueeze(-1)
    min_ind = min_ind.view(batch, num_kps, K, 1, 1).expand(
        batch, num_kps, K, 1, 2
    )
...
def ctdet_decode(heat, obb, target_mode_conf, reg=None, K=100):
    ...
    if target_mode_conf["box_mode"] == "wh":
        ...
        wh_bb = torch.cat(
            [obb[..., 1:2] + obb[..., 3:4], obb[..., 0:1] + obb[..., 2:3]],
            dim=2,
        )
    ...
    detections = torch.cat(
        [bboxes, angles * 180 / math.pi, scores, clses], dim=2
    )
