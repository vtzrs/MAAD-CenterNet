...
def get_model_info(model: nn.Module, tsize: Sequence[int]) -> str:
    ...
    stride = 64
    # Create two input tensors matching the model's input configuration
    img_source = torch.zeros(
        (1, 3, stride, stride), device=next(model.parameters()).device
    )
    img_target = torch.zeros(
        (1, 3, stride, stride), device=next(model.parameters()).device
    )
    # Profile the model with both input tensors
    flops, params = profile(
        deepcopy(model), inputs=(img_source, img_target), verbose=False
    )
    # Convert params and flops to M and Gflops respectively
    params /= 1e6
    flops /= 1e9
    # Adjust FLOPs based on the input image size
