...
from maad_centernet.models.dcn_resnet import (
    BasicBlock,
    Bottleneck,
    PoseResNet,
)
from maad_centernet.models.losses import (
    FocalLoss,
    RegL1Loss,
    RegLoss,
    VeinStemLoss,
)
from maad_centernet.utils.model_utils import _sigmoid
...
class BBModel(nn.Module):
    def __init__(self, exp):
        ...
        self.center_loss = (
            ...
            else torch.nn.MSELoss() if self.exp.center_loss == "mse" else None
        )
        self.regression_loss = (
            RegL1Loss()
            if self.exp.regression_loss == "l1"
            else RegLoss() if self.exp.regression_loss == "sl1" else None
        )
        ...
        if self.exp.target_mode_conf == "sincos":
            sincos = True
        self.vein_loss = (
            VeinStemLoss(self.exp.norm_target, sincos)
            if self.exp.vein_loss
            else self.regression_loss
        )
    ...
    def forward(
        ...
        if self.exp.target_mode_conf["do_da"]:
            predictions, x_source_feat, x_target_feat, x_source_low, x_target_low = self.model(
                x_source, x_target, alpha=1
            )
        else:
            predictions = self.model(x_source)
        ...
        if targets:
            ...
            if self.loss_weights["kphm"] > 0:
                hm_kp_loss = self.center_loss(
                    predictions["kphm"], targets["kphm"]
                )
        ...
        return predictions, loss_stats, x_source_feat, x_target_feat, x_source_low, x_target_low
