...
def draw_obbs(img, ret, target_mode_conf, img_w, down_ratio, colors):
    for k, (index, bb, offset, vis) in enumerate(
        zip(ret["ind"], ret["obb"], ret["off"], ret["reg_mask"])
    ):
        if index == 0:
            break
        if vis == 0:
...
def draw_keypoints(img, ret, target_mode_conf, img_w, down_ratio, colors):
    for k, (index, kp, offset, kp_reg_mask, vis) in enumerate(
        ...
        for stem_point in stem:
            cv2.circle(
                img, (int(stem_point[0]), int(stem_point[1])), 3, colors[k], 3
            )
...
if __name__ == "__main__":
    ...
    for j in range(0, 5, 1):
        ...
        if "kp" in ret.keys():
            draw_keypoints(
                img, ret, target_mode_conf, img_w, down_ratio, colors
            )
