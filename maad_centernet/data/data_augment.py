...
class Transform(metaclass=ABCMeta):
    def __call__(self, image, target):
        ...
        all_targets_in = np.reshape(
            all_targets_in, (-1, all_targets_in.shape[-1])
        )
        ...
        bb_visibility = np.array(
            [
                not (
                    i[0] < 0
                    or i[1] < 0
                    or i[0] >= image.shape[1]
                    or i[1] >= image.shape[2]
                )
        ...
        if num_targets == 0:
            ...
            kp_targets_out = transformed_kp_targets[:, :2].reshape(
                kp_targets.shape
            )
            kp_visibility = kp_visibility.reshape(
                (kp_targets.shape[0], kp_targets.shape[1], 1)
            )
...
class TrainTransform(Transform):
    def __init__(
        ...
        augmentation_list.append(
            A.LongestMaxSize(max_size=image_size[0], interpolation=1, p=1.0)
        )
        augmentation_list.append(
            A.PadIfNeeded(
        ...
        if p_random_brightness_contrast > 0:
            augmentation_list.append(
                A.RandomBrightnessContrast(p=p_random_brightness_contrast)
            )
        if p_normalize > 0:
            augmentation_list.append(
                A.Normalize(
